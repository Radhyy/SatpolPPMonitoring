...
export default function AnggotaClient({ initialAnggota }: { initialAnggota: Anggota[] }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (modalMode === 'ADD') {
        ...
        setAnggotaList(prev => [...prev, data].sort((a, b) => a.nama.localeCompare(b.nama)));
      } else {
        // EDIT
        const res = await fetch(`/api/anggota/${currentId}`, {
          method: 'PUT',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ nama: namaInput })
        });
        ...
        setAnggotaList(prev => prev.map(a => a.id === currentId ? data : a).sort((a, b) => a.nama.localeCompare(b.nama)));
      }
  ...
  const openDeleteModal = (id: number, nama: string) => {
    setModalMode('DELETE');
    setCurrentId(id);
    setDeleteNama(nama);
    setErrorMsg('');
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      setAnggotaList(prev => prev.filter(a => a.id !== currentId));
      closeModal();
      router.refresh();
    } catch (err: any) {
      setErrorMsg(err.message);
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left', minWidth: '500px' }}>
            <thead style={{ backgroundColor: '#f8fafc', borderBottom: '2px solid #e2e8f0' }}>
              <tr>
                ...
                <th style={{ padding: '1.25rem 1.5rem', fontWeight: 600, color: '#475569', fontSize: '0.875rem', textTransform: 'uppercase', letterSpacing: '0.5px' }}>Nama Petugas</th>
            ...
            <tbody>
              {filteredAnggota.map((anggota) => (
                <tr key={anggota.id} style={{ borderBottom: '1px solid #f1f5f9', transition: 'background-color 0.2s' }} onMouseEnter={(e) => e.currentTarget.style.backgroundColor = '#f8fafc'} onMouseLeave={(e) => e.currentTarget.style.backgroundColor = 'transparent'}>
                  <td style={{ padding: '1rem 1.5rem', fontSize: '0.9rem', color: '#64748b' }}>#{anggota.id}</td>
                  <td style={{ padding: '1rem 1.5rem', fontWeight: 600, color: '#334155' }}>{anggota.nama}</td>
                  <td style={{ padding: '1rem 1.5rem', textAlign: 'right' }}>
                    <div style={{ display: 'flex', gap: '0.5rem', justifyContent: 'flex-end' }}>
                      ...
                      <button 
                        onClick={() => openDeleteModal(anggota.id, anggota.nama)}
              ...
              {filteredAnggota.length === 0 && (
                <tr>
                  <td colSpan={3} style={{ padding: '3rem', textAlign: 'center', color: '#94a3b8' }}>
                    {anggotaList.length === 0 ? 'Belum ada data anggota terdaftar.' : 'Tidak ada nama yang sesuai dengan pencarian.'}
                  </td>
                </tr>
              )}
            </tbody>
        ...
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, backgroundColor: 'rgba(15, 23, 42, 0.6)', display: 'flex', justifyContent: 'center', alignItems: 'center', zIndex: 9999, backdropFilter: 'blur(4px)' }}>
          <div style={{ backgroundColor: 'white', borderRadius: '1rem', padding: '2rem', width: '100%', maxWidth: '400px', boxShadow: '0 25px 50px -12px rgba(0,0,0,0.5)' }}>
            <h3 style={{ fontSize: '1.25rem', fontWeight: 700, color: modalMode === 'DELETE' ? '#ef4444' : '#1e293b', marginBottom: '1.5rem', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
              ...
              {modalMode === 'ADD' ? 'Tambah Anggota Baru' : modalMode === 'EDIT' ? 'Edit Nama Anggota' : 'Konfirmasi Hapus'}
            </h3>
            
            {modalMode === 'DELETE' ? (
              <div>
                <p style={{ color: '#475569', fontSize: '0.95rem', marginBottom: '1.5rem', lineHeight: 1.5 }}>
                  Apakah Anda yakin ingin menghapus petugas bernama <strong style={{ color: '#0f172a' }}>"{deleteNama}"</strong>? Data yang sudah dihapus tidak dapat dikembalikan.
                </p>
                {errorMsg && (
